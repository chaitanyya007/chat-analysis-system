...
#basic analysis
    if st.sidebar.button("Show Analysis"):
        ...
        user_heatmap = helper.hourly_activity_map(selected_user, df)
        fig, ax = plt.subplots()
...
#finding busiest person in chat
        ...
        with col1:
            ...
            total_emojis = most_common_emojis[1].sum()
            st.title(total_emojis)
        with col2:
            st.header("Most Commonly Used Emojis")
            st.dataframe(most_common_emojis.rename(columns={0: "Emoji", 1: "Frequency"}))

        labels, sizes = helper.emoji_representation(selected_user, df)
        fig = plt.figure(figsize=(12, 8))
        squarify.plot(sizes=sizes, label=labels, alpha=0.8, color=plt.cm.viridis(range(len(sizes))))
        plt.axis('off')
        plt.title("Emoji Usage Treemap", fontsize=16)
